...
export function ArenaPage() {
  ...
  const { walletMonsters, recentMatches, activePlayers } = useAnavrinData();
  ...
  const upsertMatchQueryParam = (matchId: string) => {
    setParams((prev) => {
    ...
    >
      ...
      <div className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
        ...
        <div className="space-y-4">
          <div className="glass-card space-y-3 p-4">
            <h3 className="text-lg font-bold">Live Players</h3>
            {activePlayers.isLoading ? (
              <LoadingGrid count={2} />
            ) : (activePlayers.data ?? []).length === 0 ? (
              <div className="text-sm text-gray-400">No recent player activity found.</div>
            ) : (
              <div className="space-y-2">
                {(activePlayers.data ?? []).slice(0, 12).map((p) => (
                  <div key={p.address} className="rounded-xl border border-borderSoft bg-black/20 px-3 py-2 text-sm">
                    <div className="font-semibold">{short(p.address)}</div>
                    <div className="text-xs text-gray-400">{p.source === "match_created" ? "Created Match" : "Deposited"}</div>
                  </div>
                ))}
              </div>
            )}
