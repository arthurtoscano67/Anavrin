...
export function MartianGifCard({
  monster,
  actions,
  showLocation = true,
}: {
  ...
  return (
    <article className="glass-card card-hover overflow-hidden">
      <MartianGifImage objectId={monster.objectId} name={monster.name} className="aspect-square" />
      <div className="space-y-3 p-4">
        ...
        <div className="text-xs text-gray-400">
          {short(monster.objectId)}
          {showLocation && <> • {monster.location === "wallet" ? "Wallet" : `Kiosk ${short(monster.kioskId)}`}</>}
        </div>
