...
async function detectSameOriginHtmlLobby(
    wsEndpoint: string
  ): Promise<string | null> {
...
export function useLobby(options: UseLobbyOptions): UseLobbyResult {
  ...
  const endpoint = useMemo(
        () => resolveEndpoint(explicitEndpoint),
        [explicitEndpoint]
      );
  ...
  const [connectionState, setConnectionState] =
    ...
    const [pendingMatchStart, setPendingMatchStart] =
          useState<LobbyStartedMatch | null>(null);
        ...
        sendRaw({ type: "ping" });
                                  ...
                                  ws.onmessage = (event) => {
                                          ...
                                          if (parsed.type === "lobbyState") {
                                                    ...
                                                    setOpenMatches(
                                                                Array.isArray(parsed.openMatches) ? parsed.openMatches : []
                                                              );
                                                    setInvites(Array.isArray(parsed.invites) ? parsed.invites : []);
                                                    setRecentMatches(
                                                                Array.isArray(parsed.recentMatches) ? parsed.recentMatches : []
                                                              );
                                                    return;
                                          }
  ...
  const joinOpenMatch = useCallback(
        (matchId: string, creatorAddress: string) => {
  ...
  const announceMatchCreated = useCallback(
        (payload: { matchId: string; opponent?: string; stakeSui?: string }) => {
                if (!address) return;
                sendRaw({
                          type: "matchCreated",
                          creator: address,
